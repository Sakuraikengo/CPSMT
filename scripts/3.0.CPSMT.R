...
probCross <- 0.99 # probability (which will decide the selection intensity for cross)
...
for (lambdaRatio in lambdaRatioList) {
  ...
  for (gCorList in gCorListList) {
    ...
    dirSave <- paste0("midstream/3.0.CPSMT/", 
                      lambdaRatio, "/", 
                      typeCor, "/", 
                      gCorName, "/")
    ...
    foreach(i = 1:nRep, 
      ...
      if (!file.exists(resPath)) {
        ...
        D2True <- CalcD2(ObjectMap = map, markerEffectMat = markerEffectTrueMat, nTrait = nTrait, k = f - 1)
        ...
        if (length(midFileList) > 0) {
          ...
          lambda <- lambdaRatio * max(predU)
          predProgList <- lapply(combName, 
                                 EvalCross, 
                                 sigmaMat, 
                                 predProgMean, 
                                 probCross, 
          ...
          predProgMat <- do.call(rbind, predProgList)
        ...
        resList <- lapply(resFilePathList, function(resFilePathEach) {
          ...
          if (i == 1) {
            ...
            plotPathVar1 <- gsub(plotPath, pattern = "_u_", replacement = "_var1_")
            plotPathVar2 <- gsub(plotPath, pattern = "_u_", replacement = "_var2_")
            plotPathCov <- gsub(plotPath, pattern = "_u_", replacement = "_cov_")
            FigPredGenVar(plotPathVar1, 
                          plotPathVar2, 
                          plotPathCov, 
                          sigmaMat, 
                          sigmaTrueMat, 
          ...
          res <- c(top, top10Mean,
                   ...
                   genicVar, bulmer,
                   accuracy1, accuracy2,
                   accuracyVar1, accuracyVar2, accuracyCov, 
