...
CalcLD <- function(genoMat, qtnEach, qtlCand) {
  ...
  LD <- D^2 /
    ((freq[qtnEach] * t(freq)) * ((1 - freq[qtnEach]) * (1 - t(freq))))
...
GetPheno <- function(uMat, sigmaE, nPheno) {
  phenoList <- lapply(1:length(sigmaE), function(x) {
    residual <- mvrnorm(
      n = nPheno,
      mu = rep(0, nrow(uMat)),
      Sigma = diag(sigmaE[x], nrow = nrow(uMat))
    )
...
plotMap <- function(
    ...
    if (!missing(map2)) {
      if (is.logical(map2)) {
        ...
        plotMap(
          ...
          shift = shift,
          show.marker.names = show.marker.names,
  ...
  if (one.map) {
    ...
    if (any(n.mar1 != n.mar2)) {
      if (show.marker.names) {
        warning(
          "Can't show marker names because of different numbers of markers."
        )
        show.marker.names <- FALSE
      }
...
GP <- function(phenoMat, genoMat) {
  ...
  for (traitInd in 1:ncol(phenoMat)) {
    ...
    if (min(eigen(genoMat %*% t(genoMat))$values) < 1e-08) {
      mrkEffects <- t(genoMat) %*%
        solve(genoMat %*% t(genoMat) + diag(1e-06, nrow = nrow(genoMat))) %*%
        model$u
    } else {
...
CalcD2 <- function(ObjectMap, markerEffectMat, nTrait, k) {
  ...
  D2List <- lapply(1:ncol(varCovInd), function(combInd) {
    ...
    D2EachChrList <- lapply(unique(ObjectMap$chr), function(eachChr) {
      ...
      myDist <- sapply(1:nrow(mapEach), function(x) {
        abs(mapEach$linkMapPos[x] - mapEach$linkMapPos)
      })
...
CalcProgMean <- function(predU) {
  predProgMeanMat <- apply(predU, 2, function(predUeach) {
    predProgMeanAll <- (rep(predUeach, each = length(predUeach)) +
      rep(predUeach, length(predUeach))) /
      2
    predProgMeanMat <- matrix(
      predProgMeanAll,
      nrow = length(predUeach),
      ncol = length(predUeach)
    )
...
LP <- function(
  predProgMat, # the potential of each cross
  ...
  s_es <- paste0("S-", ed1)
  t_es <- paste0(ed2, "-T")
  c_es <- paste0("S-L_", comb[1, ], "-R_", comb[2, ], "-T")
  es <- c(s_es, t_es, c_es)
  ...
  stc_lim <- matrix(c(rep(1:length(cs), 2), rep(1, length(cs))), ncol = 3)
  s_lim <- matrix(
    c(rep(length(cs) + 1, length(s_cs)), 1:length(s_cs), rep(1, length(s_cs))),
    ncol = 3
  )
  t_lim <- matrix(
    c(
      rep(length(cs) + 2, length(s_cs)),
  ...
  ed_lim <- matrix(NA, nrow = node_num, ncol = 3)
  for (i in 1:length(ed)) {
    ...
    ed_lim_mat <- matrix(
      c(
        rep(ed_lim_ind, length(ed_lim_each)),
        ed_lim_each,
        rep(1, length(ed_lim_each))
      ),
      ncol = 3
    )
    start <- min(which(is.na(ed_lim[, 1])))
  ...
  sym_lim_start <- matrix(
    c(sym_lim_row_ind, 1:length(s_cs), rep(1, length(s_cs))),
    ncol = 3
  )
  sym_lim_end_ind <- (1 + length(s_cs)):(length(s_cs) + length(t_cs))
  sym_lim_end <- matrix(
    c(sym_lim_row_ind, sym_lim_end_ind, rep(1, length(t_cs))),
    ncol = 3
  )
...
CreateF8 <- function(
  ...
  for (l in 2:7) {
    selfTable <- data.frame(
      ind1 = rownames(selfNowPop$genoMat),
    ...
    selfNextPop <- population$new(
      name = paste0("F", l + 1, " offspring"),
  ...
  geneticValEstimatedNew <- (genoMatNew[, marker] - 1) %*%
    markerEffectEstimated +
    betaMat
  ...
  geneticCov <- sapply(1:ncol(covPairsIdx), function(cIdx) {
    ...
    var(geneticValTrueNew[, t1], geneticValTrueNew[, t2]) *
      (nrow(geneticValTrueNew) - 1) /
      nrow(geneticValTrueNew)
  })
...
FigPredU <- function(gpPath, trueU, predU) {
  ...
  plot(
    trueU[, 1],
    predU[, 1],
    xlab = "True Genotypic Value of Trait1",
    ylab = "Predicted Genotypic Value of Trait1",
    xlim = xlim,
    ylim = ylim,
  ...
  plot(
    trueU[, 2],
    predU[, 2],
    xlab = "True Genotypic Value of Trait2",
    ylab = "Predicted Genotypic Value of Trait2",
    xlim = xlim,
    ylim = ylim,
...
FigPredMarker <- function(
  ...
  par(mfrow = c(1, 2))
  xlim <- ylim <- range(markerEffectEstimatedMat[, 1], markerEffectTrueMat[, 1])
  plot(
    markerEffectTrueMat[!alleleFixed, 1],
    markerEffectEstimatedMat[!alleleFixed, 1],
    xlab = "True Marker Effects of Trait1",
    ylab = "Estimated Marker Effects of Trait1",
    xlim = xlim,
    ylim = ylim,
    main = paste0(
      "r = ",
      round(corMarker1, 2),
      ", nMarker = ",
      nMarker - nFixedAllele
    )
  )
  abline(a = 0, b = 1, lty = 2, col = "red")

  xlim <- ylim <- range(markerEffectEstimatedMat[, 2], markerEffectTrueMat[, 2])
  plot(
    markerEffectTrueMat[!alleleFixed, 2],
    markerEffectEstimatedMat[!alleleFixed, 2],
    xlab = "True Marker Effects of Trait2",
    ylab = "Estimated Marker Effects of Trait2",
    xlim = xlim,
    ylim = ylim,
    main = paste0(
      "r = ",
...
# CPS-MT score under multi-trait constraints (eq.S26-S29).
# meanMat : K x J matrix of predicted progeny means.
# varMat  : K x (J + J*(J-1)/2) matrix; first J columns are variances
#           (trait1..traitJ), remaining columns are covariances in the
#           order produced by combn(1:J, 2). For J = 3:
#           var1, var2, var3, cov12, cov13, cov23.
# w       : weight on target trait (scalar in [0, 1])
# lVec, hVec : numeric vectors of length J - 1 with bounds for each
#           essential trait (in trait order 2, 3, ..., J)
# meanPredU, sdPredU : numeric vectors of length J used to standardise
#           lambda (sigma_jt in the algorithm)
# hardConstraint : if TRUE, samples violating any essential bound are
#           assigned -10000 to act as a strict filter (selfing case).
EvalByMC <- function(
  meanMat,
  varMat,
  w,
  ...
  z <- matrix(rnorm(nMC * nTrait), nrow = nMC, ncol = nTrait)
  covPairs <- combn(1:nTrait, 2)

  mcResult <- sapply(1:nrow(meanMat), function(k) {
    muEach <- meanMat[k, ]
    sigmaMat_k <- matrix(0, nrow = nTrait, ncol = nTrait)
    for (tr in 1:nTrait) {
      sigmaMat_k[tr, tr] <- varMat[k, tr]
    }
    for (cIdx in 1:ncol(covPairs)) {
      t1 <- covPairs[1, cIdx]
      t2 <- covPairs[2, cIdx]
      sigmaMat_k[t1, t2] <- varMat[k, nTrait + cIdx]
      sigmaMat_k[t2, t1] <- varMat[k, nTrait + cIdx]
    }

    diagVar <- diag(sigmaMat_k)
    nearZero <- diagVar < 1e-6

    # Deterministic point case
    # All trait variances are ~0 (e.g. cross of genetically identical parents
    # or selfing of a fully homozygous line). Skip MC sampling and evaluate
    # lambda directly at mu: lambda1 = (mu1 - meanPredU[1]) / sdPredU[1] and,
    # for each essential trait j, lambda_j = 0 if mu_j in [l_j, h_j] else the
    # standardised distance to the nearest bound. Return -10000 under hard
    # constraint if any bound is violated, else CPS-MT eq.S26 at this point.
    if (all(nearZero)) {
      lambda1 <- (muEach[1] - meanPredU[1]) / sdPredU[1]
      lambdaEssential <- sapply(1:(nTrait - 1), function(eIdx) {
        traitIdx <- eIdx + 1
        if (muEach[traitIdx] >= lVec[eIdx] & muEach[traitIdx] <= hVec[eIdx]) {
          return(0)
        }
        max(
          (muEach[traitIdx] - hVec[eIdx]) / sdPredU[traitIdx],
          (lVec[eIdx] - muEach[traitIdx]) / sdPredU[traitIdx]
        )
      })
      if (hardConstraint) {
        if (any(lambdaEssential > 0)) {
          return(-10000)
        }
        return(lambda1)
      }
      return(w * lambda1 - (1 - w) * mean(lambdaEssential))
    }

    # Regularise covariance matrix for Cholesky
    for (tr in 1:nTrait) {
      sigmaMat_k[tr, tr] <- max(sigmaMat_k[tr, tr], 1e-6)
    }
    for (cIdx in 1:ncol(covPairs)) {
      t1 <- covPairs[1, cIdx]
      t2 <- covPairs[2, cIdx]
      maxCov <- sqrt(sigmaMat_k[t1, t1] * sigmaMat_k[t2, t2]) * (1 - 1e-6)
      sigmaMat_k[t1, t2] <- max(min(sigmaMat_k[t1, t2], maxCov), -maxCov)
      sigmaMat_k[t2, t1] <- sigmaMat_k[t1, t2]
    }

    # Cholesky factor L (Sigma = L^T L) is used to turn the iid N(0, 1)
    # draws z into MVN(0, Sigma) samples via z %*% L, then shifted by mu
    # to get x ~ MVN(mu, Sigma) -- the predicted progeny distribution.
    L <- tryCatch(
      chol(sigmaMat_k),
      error = function(e) chol(sigmaMat_k + diag(1e-6, nTrait))
    )
    x <- z %*% L
    for (tr in 1:nTrait) {
      x[, tr] <- x[, tr] + muEach[tr]
    ...
    # lambda for target trait (eq.S28)
    lambda1 <- (x[, 1] - meanPredU[1]) / sdPredU[1]

    # lambda for essential traits (eq.S29)
    lambdaEssentialMat <- matrix(0, nrow = nMC, ncol = nTrait - 1)
    for (eIdx in 1:(nTrait - 1)) {
      traitIdx <- eIdx + 1
      xEach <- x[, traitIdx]
      inRange <- xEach >= lVec[eIdx] & xEach <= hVec[eIdx]
      lambdaEssentialMat[, eIdx] <- ifelse(
        inRange,
        0,
        pmax(
          (xEach - hVec[eIdx]) / sdPredU[traitIdx],
          (lVec[eIdx] - xEach) / sdPredU[traitIdx]
        )
      )
    }
    avgLambdaEssential <- rowMeans(lambdaEssentialMat)

    if (hardConstraint) {
      allInRange <- rep(TRUE, nMC)
      for (eIdx in 1:(nTrait - 1)) {
        traitIdx <- eIdx + 1
        allInRange <- allInRange &
          (x[, traitIdx] >= lVec[eIdx] & x[, traitIdx] <= hVec[eIdx])
      }
      x1hard <- ifelse(allInRange, lambda1, -10000)
      threshold <- quantile(x1hard, probs = prob)
      return(min(x1hard[x1hard >= threshold]))
    }

    # eq.S26
    penalizedVal <- w * lambda1 - (1 - w) * avgLambdaEssential
